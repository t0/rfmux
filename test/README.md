...
| --- | --- | --- | --- |
| `pytest --tier=portable` | 740 | varies | Changing packaging, dependencies, or the Python floor. This is what `tox` runs on 3.10-3.12. |
| `pytest --tier=quick` | 2037 | varies | Default while editing. |
...
| `pytest --tier=full` | 2074 | varies | Before pushing. Everything that runs without a board, the acquisition tier included. |
| `pytest --tier=hardware --serial 0024` | 75 | needs a board | Against a connected board; see *Hardware tests*. |
| `pytest --tier=all --serial 0024` | 2149 | needs a board | Before a release. |
