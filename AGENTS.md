...
- Single convergence loop: `converged_lekid_parameters()`, seeded with each
  tone's last currents (the model's branch memory) and stepped adaptively,
...
```bash
pytest --tier=quick                 # Edit loop: 1019 tests, ~1 min
pytest --tier=portable              # No CRS, no GUI: 43 tests, ~9 s
pytest --tier=full                  # All 1041 that run without a board, ~4 min
