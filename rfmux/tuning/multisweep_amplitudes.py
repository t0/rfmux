"""Sweeping one array at a ladder of amplitudes: deciding the amplitudes.

:class:`AmplitudeSchedule` answers one question — *at what amplitude is each
resonator swept, on each pass?* — and answers it without a board.  The loop that
walks the answer and calls ``crs.multisweep`` once per pass lives in
``rfmux.algorithms.measurement.multisweep_amplitudes``; this half is pure, so a
schedule can be built, printed, validated and unit-tested with no hardware and
no GUI in sight.
...
__all__ = ["AmplitudeSchedule", "AmplitudeStep"]
