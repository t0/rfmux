...
| --- | --- | --- | --- |
| `pytest --tier=portable` | 690 | varies | Changing packaging, dependencies, or the Python floor. This is what `tox` runs on 3.10-3.12. |
| `pytest --tier=quick` | 1971 | varies | Default while editing. |
...
| `pytest --tier=full` | 2008 | varies | Before pushing. Everything that runs without a board, the acquisition tier included. |
| `pytest --tier=hardware --serial 0024` | 75 | needs a board | Against a connected board; see *Hardware tests*. |
| `pytest --tier=all --serial 0024` | 2083 | needs a board | Before a release. |
