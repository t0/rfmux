...
| --- | --- | --- | --- |
| `pytest --tier=portable` | 839 | varies | Changing packaging, dependencies, or the Python floor. This is what `tox` runs on 3.10-3.12. |
| `pytest --tier=quick` | 2329 | varies | Default while editing. |
| `pytest --tier=acquisition` | 40 | varies | After changing streaming, decimation, the PFB path, or pulse capture. A subset of `full`: run one or the other, not both. |
| `pytest --tier=full` | 2369 | varies | Before pushing. Everything that runs without a board, the acquisition tier included. |
| `pytest --tier=hardware --serial 0024` | 75 | needs a board | Against a connected board; see *Hardware tests*. |
| `pytest --tier=all --serial 0024` | 2442 | needs a board | Before a release. |
