...
- **Noise Spectrum**
  ...
  5. Calibrated measurements save `df_hz`, `psd_df`, and `psd_dissipation` beside each stream's unchanged native IQ and helper spectra. The viewer reads those scientific products directly and keeps mean subtraction and carrier-bin omission as display-only operations. PFB conversion requires the recorded NCO frequency. Legacy noise payloads are not supported.
