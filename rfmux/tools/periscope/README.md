...
- **Find Bias**
  ...
  2. The report becomes the panel's catalog, and the sweeps come back carrying it as `bias_report`. On the sweep grids the amplitude step each resonator is biased at is drawn thick, with a dashed line at its bias frequency — dashed because solid and dotted already mean upward and downward. That line is named in the legend, `f_bias` over the drive it was chosen at, in whatever units the panel is displaying; on a resonator whose point is a fallback rather than a measurement the row reads `f_bias — FLAGGED`, and the subplot's tooltip says why. The counts and the flagged names also go to the status line, which fades like any other outcome — the flags themselves are on the subplots they belong to.
