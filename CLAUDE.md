...
```bash
pytest --tier=quick                 # Edit loop: 661 tests, ~1 min, zero skips
pytest --tier=portable              # No CRS, no GUI: 37 tests, ~9 s
pytest --tier=full                  # All 677 that run without a board, ~4 min
...
so don't add it by hand. A bare `pytest` still runs 677 + ~75 hardware skips.
