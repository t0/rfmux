...
```bash
pytest --tier=quick                 # Edit loop: 659 tests, ~1 min, zero skips
pytest --tier=portable              # No CRS, no GUI: 37 tests, ~9 s
pytest --tier=full                  # All 675 that run without a board, ~4 min
...
so don't add it by hand. A bare `pytest` still runs 675 + ~75 hardware skips.
