...
| `pytest --tier=acquisition` | 16 | ~8.5 min | After changing streaming, decimation, the PFB path, or pulse capture. A subset of `full`: run one or the other, not both. |
| `pytest --tier=full` | 553 | ~9.5 min | Before pushing. Everything that runs without a board, the acquisition tier included. |
