...
def test_slow_stream_survives_a_tone_far_below_every_resonator():
    """At 1 Hz the envelope model's tangent dS21/dI is a difference of
    two identical currents."""
    crs = _unbiased_mock()
    asyncio.run(crs.set_amplitude(0.01, channel=1, module=1))
    asyncio.run(crs.set_frequency(1.0, channel=1, module=1))
