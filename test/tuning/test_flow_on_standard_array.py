...
where the drivers actually run: one multisweep over an amplitude ladder on the
standard array (``rfmux.mock.standard_array``), then bias finding on what came
back, then biasing the board with the result. The array and the ladder are
built once for the module — the ladder is the expensive step, about half a
...
#: The ladder the standard array is swept over: half to eight times the
#: simulator's own bias amplitude, five rungs. Wide enough that every
#: resonator bifurcates inside it, and narrow enough that the answer is a rung.
LADDER = AmplitudeSchedule.multiplicative(0.5, 8.0, 5)
TOP = len(LADDER.ladder) - 1


@pytest.fixture(scope="module")
def ladder_sweeps(standard_array_board):
    """One module's multisweep over LADDER, both directions."""
...
@pytest.fixture(scope="module")
def bias_report(ladder_sweeps):
    with warnings.catch_warnings():
        warnings.simplefilter("error", UserWarning)
        return find_bias_points(ladder_sweeps, save=False)


def _fitted_a(ladder_sweeps, name, rung):
    p = ladder_sweeps["results"][rung]["upward"][name]["fits"]["nonlinear"]["params"]
    return p["a"] if p else float("nan")


@pytest.fixture(scope="module")
def nonlinear_fits(ladder_sweeps):
    report = fit_sweeps(ladder_sweeps, models=("nonlinear",))
    return report
...
def test_a_sweep_entry_is_a_measurement_and_nothing_else(ladder_sweeps, standard_array_board):
    ...
    for rung in range(TOP + 1):
        for direction in ("upward", "downward"):
            for name in catalog.names():
                entry = ladder_sweeps["results"][rung][direction][name]
...
def test_every_resonator_gets_a_bias_point_at_a_ladder_rung(bias_report, ladder_sweeps, standard_array_board):
    _, _, catalog = standard_array_board
    ...
    for f in bias_report.findings:
        entry = ladder_sweeps["results"][f.iteration]["upward"][f.name]
...
def test_bias_finding_on_a_bifurcating_ladder_raises_no_warning(bias_report):
    """Bifurcation inside the ladder is the expected outcome, not a warning;
...
def test_every_resonator_bifurcates_inside_the_ladder(bias_report):
    """The array and ladder are chosen so the finder always has something to
    find. A resonator that never bifurcated would make the loudest rung the
...
def test_the_nonlinearity_rises_with_drive(nonlinear_fits, ladder_sweeps, standard_array_board):
    _, _, catalog = standard_array_board
    assert not nonlinear_fits.failed, nonlinear_fits.failed
    for name in catalog.names():
        assert _fitted_a(ladder_sweeps, name, 0) < _fitted_a(ladder_sweeps, name, TOP - 1), name


def test_the_ladder_brackets_bifurcation_for_every_resonator(nonlinear_fits, ladder_sweeps, standard_array_board):
    """Below the bifurcation value one rung from the top, above it at the top:
    the property the ladder is chosen for."""
    _, _, catalog = standard_array_board
    for name in catalog.names():
        assert _fitted_a(ladder_sweeps, name, TOP - 1) < BIFURCATION_A < _fitted_a(ladder_sweeps, name, TOP), name


def test_the_pull_is_downward(nonlinear_fits, ladder_sweeps, standard_array_board):
    ...
    further below it than when driven gently. At the quiet rung the pull is a
    few hertz and the 1 kHz sweep grid decides which side the minimum lands
    on, so only the ordering is asserted there."""
    _, _, catalog = standard_array_board

    def pull_hz(name, rung):
        entry = ladder_sweeps["results"][rung]["upward"][name]
