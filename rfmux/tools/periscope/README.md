...
- **Edit catalog**
  ...
  5. To remove resonators by name, enter exact, case-sensitive names in **Remove resonators by name**, separated by commas or newlines, then press **Remove names and re-sweep**. Duplicate names count once. Unknown names and requests leaving no survivors are reported in the dialog. This opens the same multisweep configuration with a reduced copy of the catalog; canceling leaves the original unchanged. Manual re-sweep is also unavailable for foreign-module files or capture tuning.

- **Sweep centers**

  Sweep centers are independent of catalog bias points. Choose **Catalog bias frequencies**, **Previous multisweep centers**, or **Custom frequencies**. With an active catalog, enter one custom center per resonator in the displayed order; names, channels, bias frequencies, amplitudes and calibrations are preserved in the sweep's catalog snapshot. Without a catalog, custom frequencies seed a new array. In Python, pass `center_frequencies={name: center_hz, ...}` alongside the catalog, covering every resonator. Each measured trace records its actual `original_center_frequency`; Find Bias updates the returned catalog from those traces.
...
- **Find Bias**
  ...
  2. The report becomes the panel's catalog, and the sweeps come back carrying it as `bias_report`. On the sweep grids the amplitude step each resonator is biased at keeps its normal line and gains a translucent wide overlay, with a dashed line at its bias frequency — dashed because solid and dotted already mean upward and downward. That line is named in the legend, `f_bias` over the drive it was chosen at in normalized DAC units — what a bias amplitude is, and what goes back into a re-run. On a resonator whose point is a fallback rather than a measurement the row names the flag, `f_bias — freq out of bounds`, and the subplot's tooltip carries the sentence behind it. The words are the library's `FLAG_KINDS`, so a plot and a notebook call a flag the same thing. The status line says `Bias found (2 of 9 flagged)`, then fades like any other outcome — nothing is on the board until Apply Bias, and which resonators are flagged is on the subplots they belong to.
  3. The **Bias: derivative** tab shows point-to-point changes in normalized IQ speed, divided by each curve's prominence threshold. The ±1 lines mark a change equal to one threshold above or below zero; the vertical scale is dimensionless. The threshold is the larger of the spike and noise gates. Faint lines show the smaller gate at the selected drive. These are reference lines, not a pass/fail band: the detector checks prominence relative to the surrounding base and requires a nearby positive/negative spike pair. The plot is available before Find Bias and redraws when you apply Find Bias Settings. Open **Help → Bias plot guide** for explanations of the derivative, hysteresis and frequency panels, including their scales and legends.
...
- **Noise Spectrum**
  1. After **Apply Bias**, choose **Get Noise Spectrum** to measure the catalog's named resonators. The main window's **Noise Spectrum** button measures explicitly selected channels. Both run `measure_noise` and save its current-format noise container in the session folder. Keep the slow stream running; a requested decimation change remains in effect after the measurement.
  2. Open the saved file from the Session Browser to see the same viewer offline. **Timestreams** and **Power spectral densities** each show one subplot per resonator, with pages of 20 and up to five columns. Choose **Slow** or **PFB** when both captures are present. PFB times are relative to each separate capture, so they do not align across resonators or with the slow capture.
  3. **Volts** shows I/Q in V and PSDs in V²/Hz. **df** is available when the saved catalog carries nonzero `dI_df + j*dQ_df`; it shows df and the orthogonal dissipation component in Hz, with PSDs in Hz²/Hz. The complex conversion is `IQ_volts / (dI_df + j*dQ_df)`.
  4. **Subtract timestream mean** starts checked and affects only the timestream display. PSDs use logarithmic axes and omit the zero-frequency carrier bin. Saved arrays remain unchanged.
  5. New measurements save linear voltage and calibrated df spectra in each stream's `display_psds`, so the viewer reads them directly. For current-format files without these products, a headless helper reconstructs only the spectra needed for the selected units, using the recorded acquisition settings. PFB reconstruction requires the recorded NCO frequency. Legacy noise payloads are not supported.
