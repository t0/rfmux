...
| `pytest --tier=quick` | 548 | ~50 s | Default while editing. |
...
| `pytest --tier=full` | 564 | ~4 min | Before pushing. Everything that runs without a board, the acquisition tier included. |
| `pytest --tier=hardware --serial 0024` | 75 | needs a board | Against a connected board; see *Hardware tests*. |
| `pytest --tier=all --serial 0024` | 639 | needs a board | Before a release. |
